...
// command line). API Key and platform userToken live in the DSH credentials
// service. Every route is loopback + same-origin guarded; balance/usage are
// cached with in-flight dedup.
...
const LEGACY_STORE_PATH = join(homedir(), '.dsh', 'ds-balance.json')
...
const MAX_ZIP_OUTPUT_BYTES = 32 * 1024 * 1024
...
// ---- one-time migration from the plugin's legacy plaintext store ----
async function readLegacyToken() {
  try {
    const text = await fs.readFile(LEGACY_STORE_PATH, 'utf8')
    const parsed = JSON.parse(text)
    return typeof parsed.userToken === 'string' && parsed.userToken.trim() !== '' ? parsed.userToken.trim() : null
  } catch {
    return null
  }
}
...
async function removeLegacyStore() {
  try { await fs.rm(LEGACY_STORE_PATH, { force: true }) } catch { /* best-effort cleanup */ }
}
...
async function readBodyLimited(res, maxBytes) {
  ...
  try {
    while (true) {
      ...
      if (size > maxBytes) {
        await reader.cancel()
        throw codedError(ERR.INVALID_RESPONSE, '响应过大')
      }
...
async function fetchDeepSeekBuffer(url, headers, timeoutMs = 30000) {
  ...
  if (!reader) {
    ...
    if (buf.length > MAX_EXPORT_BYTES) throw codedError(ERR.INVALID_RESPONSE, '导出文件过大')
    return buf
  }
  const chunks = []
  let size = 0
  try {
    while (true) {
      ...
      if (size > MAX_EXPORT_BYTES) {
        await reader.cancel()
        throw codedError(ERR.INVALID_RESPONSE, '导出文件过大')
      }
      chunks.push(Buffer.from(part.value))
    }
  } finally {
...
function extractZipCsv(buffer) {
  ...
  if (count > MAX_ZIP_ENTRIES) throw codedError(ERR.INVALID_RESPONSE, 'zip 条目过多')
  let offset = buffer.readUInt32LE(eocd + 16)
  const files = {}
  let outputBytes = 0
  for (let n = 0; n < count; n++) {
    if (offset < 0 || offset + 46 > buffer.length || buffer.readUInt32LE(offset) !== 0x02014b50) {
      throw codedError(ERR.INVALID_RESPONSE, 'zip 目录结构异常')
    }
    ...
    if (nameLen > buffer.length - offset - 46 || extraLen > buffer.length - offset - 46 - nameLen) {
      throw codedError(ERR.INVALID_RESPONSE, 'zip 条目边界异常')
    }
    const name = buffer.toString('utf8', offset + 46, offset + 46 + nameLen)
    if (uncompSize > MAX_ZIP_OUTPUT_BYTES || outputBytes + uncompSize > MAX_ZIP_OUTPUT_BYTES) {
      throw codedError(ERR.INVALID_RESPONSE, 'zip 解压内容过大')
    }
    if (localOffset + 30 > buffer.length) throw codedError(ERR.INVALID_RESPONSE, 'zip 本地条目异常')
    const lhNameLen = buffer.readUInt16LE(localOffset + 26)
    const lhExtraLen = buffer.readUInt16LE(localOffset + 28)
    const dataStart = localOffset + 30 + lhNameLen + lhExtraLen
    if (dataStart > buffer.length || compSize > buffer.length - dataStart) throw codedError(ERR.INVALID_RESPONSE, 'zip 压缩数据异常')
    const compressed = buffer.subarray(dataStart, dataStart + compSize)
    let data
    try {
      data = method === 0 ? compressed : method === 8 ? inflateRawSync(compressed, { maxOutputLength: MAX_ZIP_OUTPUT_BYTES - outputBytes }) : null
    } catch { throw codedError(ERR.INVALID_RESPONSE, 'zip 解压失败') }
    if (data === null) throw codedError(ERR.INVALID_RESPONSE, 'zip 压缩方式不支持')
    outputBytes += data.length
    if (outputBytes > MAX_ZIP_OUTPUT_BYTES) throw codedError(ERR.INVALID_RESPONSE, 'zip 解压内容过大')
...
export function apply(ctx) {
  ...
  async function resolveToken() {
    try {
      let cred = await ctx.credentials.resolve(USER_TOKEN_REF)
      if (cred && typeof cred.value === 'string' && cred.value.trim()) {
        await removeLegacyStore()
        return { token: cred.value.trim(), source: cred.source }
      }
      const legacyToken = await readLegacyToken()
      if (!legacyToken) return { token: null, source: null }
      await ctx.credentials.set(USER_TOKEN_REF, legacyToken)
      await removeLegacyStore()
      cred = await ctx.credentials.resolve(USER_TOKEN_REF)
      const token = cred && typeof cred.value === 'string' ? cred.value.trim() : ''
      return token ? { token, source: cred.source } : { token: null, source: null }
    } catch {
  ...
  function guard(req, res, method, requireOrigin = false) {
    if (!isLoopbackRequest(req, requireOrigin)) { json(res, 403, { ok: false, code: ERR.FORBIDDEN, message: 'forbidden' }); return false }
    ...
    },
    {
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT') {
          ...
          try {
            await ctx.credentials.set(USER_TOKEN_REF, token)
            await removeLegacyStore()
            usageFetcher.clear()
            allTimeFetcher.clear()
            usageExportFetcher.clear()
            todayFetcher.clear()
            json(res, 200, { ok: true, hasToken: true, maskedToken: maskSecret(token) })
          } catch (e) {
        ...
        if (req.method === 'DELETE') {
          ...
          try {
            await ctx.credentials.unset(USER_TOKEN_REF)
            await removeLegacyStore()
            usageFetcher.clear()
            allTimeFetcher.clear()
            usageExportFetcher.clear()
            todayFetcher.clear()
            json(res, 200, { ok: true, hasToken: false })
          } catch (e) {
            json(res, 502, { ok: false, code: ERR.FETCH_FAILED })
          }
          return
