...
async function fetchDeepSeek(url, headers, timeoutMs = REQUEST_TIMEOUT_MS) {
  let res
  try {
    res = await fetch(url, {
...
async function fetchTodayUsage(token) {
  ...
  const nextHourSec = Math.ceil(now.getTime() / 3600000) * 3600 / 1000
...
async function fetchAllTimeTotal(token) {
  if (!token) throw codedError(ERR.MISSING_TOKEN, '未配置 userToken')
  const now = new Date()
  ...
  for (let i = 0; i < 48; i++) {
    ...
    const { status, data } = await fetchDeepSeek(url, { authorization: 'Bearer ' + token, 'user-agent': UA, referer: REFERER }, ALLTIME_REQUEST_TIMEOUT_MS)
...
export function apply(ctx) {
  ...
  const allTimeFetcher = createCachedFetcher(async () => fetchAllTimeTotal((await resolveToken()).token), ALLTIME_TTL_MS)
